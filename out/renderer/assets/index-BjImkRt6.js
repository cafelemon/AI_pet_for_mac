...
function motionForAction(action, statesConfig) {
  return statesConfig.motions?.[action.id] ?? {
    ...DEFAULT_MOTION,
    playback: action.playback
  };
}
function renderableAction(action) {
  return Boolean(action?.runtime && action.available);
}
function buildKeyframes(companionConfig, statesConfig, actionRegistry) {
  const actionIds = statesConfig.pa0KeyframeFolders.length > 0 ? statesConfig.pa0KeyframeFolders : actionRegistry.actionOrder;
  const fallbackAction = actionRegistry.actions[actionRegistry.fallbackAction];
  return actionIds.map((actionId) => actionRegistry.actions[actionId] ?? fallbackAction).filter(renderableAction).map((action) => {
    const sourceVideoPaths = action.sourceVideoPaths.length > 0 ? action.sourceVideoPaths : [`${action.sourceDir}/${action.id}_source.mp4`];
    return {
      folder: action.id,
...
function controlGroupForAction(action) {
...
function buildActionGroups(byFolder, actionRegistry) {
...
function buildCompanionCatalog(keyframes, statesConfig, actionRegistry) {
...
function ControlCenterApp() {
  ...
  reactExports.useEffect(
    () => window.companionAPI.onPetProfileChanged((state) => {
      ...
      loadControlCenter().catch((error) => console.error("Failed to reload pet profile", error));
    }),
    [loadControlCenter]
  );
